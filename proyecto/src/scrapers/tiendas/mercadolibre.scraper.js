    ...
    try {
      ...
      while (todosLosProductos.length < maxProductos) {
        try {
          ...
          console.error(`[MercadoLibre] Error al procesar página ${pagina}:`, pageError.message);
  ...
  async scrapeUrl(url) {
    try {
      ...
      const producto = await this.page.evaluate(() => {
        ...
        let marca = '';
        // Buscar Marca en la tabla de especificaciones
        const rows = Array.from(document.querySelectorAll('tr.ui-vpp-striped-specs-detail__item, tr.ui-pdp-specs__table__row, .ui-pdp-specs__table tr'));
        for (const row of rows) {
          if (row.innerText && row.innerText.includes('Marca')) {
        ...
        if (!marca) {
          const brandEl = document.querySelector('.ui-pdp-brand-link') || document.querySelector('[class*="brand"]');
          if (brandEl) {
            marca = brandEl.innerText.replace(/Ver más productos marca\s+/gi, '').trim();
          }
        }

        let imagen = '';
        const imgEl = document.querySelector('img.ui-pdp-gallery__figure__image') || document.querySelector('img.ui-pdp-image');
        if (imgEl) {
